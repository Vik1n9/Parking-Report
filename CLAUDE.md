...
- `public/js/parking-core.js` — **唯一**格式化邏輯來源（標籤、解析、LINE/口播稿/Excel 值、車塔使用率）。ESM，Worker 與瀏覽器共用同一份，禁止在別處重複實作
- `src/index.js` — Worker 進入點：非 `/api/*` 走靜態資產，`/api/*` 走路由
- `src/auth.js` — 保全裝置 token（Bearer，SHA-256 存 D1）+ Cloudflare Access JWT 驗證
- `src/routes/` — zones（公開）、reports（送出/佇列/確認/退回）、records（查詢）
...
2. **tokens 儲存格式**：`{zone_code: value}` 物件（非位置陣列），值語意：整數=剩餘車位(車區)/成數(其他區)、`0`=全滿、`x`=未停車、`0.N`=台車
3. **伺服器重算**：確認時由 `reports.tokens_json` 重算所有快照，`raw_input` 僅稽核
4. **保全頁離線優先**：LINE 分享/複製 100% 本機可用；API 送出進 localStorage 佇列重試，永不阻塞
5. 測試必須雙時區通過：`TZ=Asia/Taipei` 與 `TZ=UTC`
